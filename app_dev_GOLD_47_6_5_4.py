...
def pe47_quick_navigation():
    ...
    links += (
        '<a class="pe47-nav-button" '
        'href="?market=gold" target="_self">GOLD</a>'
    )
...
try:
    PE_ACTIVE_MARKET = str(
        st.query_params.get("market", "oil")
    ).strip().lower()
except Exception:
    PE_ACTIVE_MARKET = "oil"

if PE_ACTIVE_MARKET not in {"oil", "gold"}:
    PE_ACTIVE_MARKET = "oil"

st.set_page_config(
    page_title=(
        "PROCUREYE | Gold Market Intelligence"
        if PE_ACTIVE_MARKET == "gold"
        else "PROCUREYE | Oil Market Intelligence"
    ),
    page_icon="🟡" if PE_ACTIVE_MARKET == "gold" else "🛢️",
    layout="wide",
...
_pe_hero_title = (
    ...
    if PE_ACTIVE_MARKET == "gold"
    else "Crude Oil Market Intelligence Platform"
)

_pe_hero_copy = (
    "Decision-support intelligence for Gold in USD using market "
    "structure, momentum, volatility, the US dollar, yields, news "
    "and explainable reasoning."
    if PE_ACTIVE_MARKET == "gold"
    else
    "Decision-support intelligence for Brent and WTI using market "
    "structure, momentum, volatility, news, historical memory and "
...
<section class="pe-hero">
  <div class="pe-top">
    ...
    <div class="pe-release">Release 47.6.5.4 DEV
  </div>
  <div class="pe-title">{_pe_hero_title}</div>
  <div class="pe-copy">
    {_pe_hero_copy}
  </div>
  <div class="pe-badges">
...
# ============================================================
# PROCUREYE RELEASE 47.6.5.4 DEV — GOLD INTELLIGENCE PAGE
# ============================================================
...
GOLD_NEWS_FEEDS = [
    (
        ...
        "https://news.google.com/rss/search?q="
        + quote_plus(
            '(gold OR XAU OR "gold price") '
            '(Federal Reserve OR dollar OR yields OR inflation) when:3d'
        )
        + "&hl=en-US&gl=US&ceid=US:en"
    ),
    (
        ...
        "https://news.google.com/rss/search?q="
        + quote_plus(
            '(gold reserves OR "central bank gold" OR "gold buying") '
            'when:7d'
        )
        + "&hl=en-US&gl=US&ceid=US:en"
    ),
    (
        ...
        "https://news.google.com/rss/search?q="
        + quote_plus(
            '(gold safe haven OR geopolitical risk gold) when:3d'
        )
        + "&hl=en-US&gl=US&ceid=US:en"
    ),
]
...
def _gold_news_bias(text):
    value = str(text).lower()

    bullish = [
        "rate cut", "lower yields", "falling yields", "dollar weak",
        "weaker dollar", "central bank buying", "gold buying",
        "safe haven", "geopolitical risk", "war", "conflict",
        "inflation fears", "record high", "gold demand", "outflows slow"
    ]
    bearish = [
        "rate hike", "higher yields", "rising yields", "dollar strong",
        "stronger dollar", "hawkish", "easing tensions", "gold outflows",
        "profit taking", "selloff", "gold demand falls", "inflation cools"
    ]

    positive = sum(1 for phrase in bullish if phrase in value)
    negative = sum(1 for phrase in bearish if phrase in value)
    raw = (positive - negative) * 24
    impact = max(-100.0, min(100.0, float(raw)))
...
def get_gold_market_news(limit=3):
    ...
    for feed_name, url in GOLD_NEWS_FEEDS:
        try:
        ...
        for entry in parsed.entries[:25]:
            ...
            bias, impact = _gold_news_bias(title + " " + summary)
            ...
            rows.append({
                "Title": title,
                "Source": source,
                "Published UTC": published_dt.strftime("%d %b %Y · %H:%M"),
                "Bias": bias,
                "Impact": impact,
                "URL": str(entry.get("link", "")).strip(),
    ...
    if not rows:
        return pd.DataFrame(columns=[
            "Title", "Source", "Published UTC", "Bias",
            "Impact", "URL", "Timestamp", "Dedup"
        ])
    ...
    news = (
        news.sort_values(
            ...
            if series.name == "Impact" else series,
            ascending=False
        )
        .drop_duplicates("Dedup")
        .head(limit)
...
def build_gold_signal(gold, dollar, yield_10y, news):
    score = 50.0

    if gold.get("trend") == "BULLISH":
        score += 14
    elif gold.get("trend") == "BEARISH":
        score -= 14

    score += max(-15, min(15, float(gold.get("momentum", 0) or 0) * 1.5))
    score -= max(-8, min(8, float(dollar.get("change", 0) or 0) * 4))
    score -= max(-8, min(8, float(yield_10y.get("change", 0) or 0) * 3))

    news_score = 0.0
    if isinstance(news, pd.DataFrame) and not news.empty:
        news_score = float(
            pd.to_numeric(news["Impact"], errors="coerce")
            .fillna(0)
            .mean()
        )
        score += max(-12, min(12, news_score * 0.12))
    ...
    confidence_score = int(max(40, min(85, 50 + distance * 1.4)))
    confidence = (
        "HIGH" if confidence_score >= 70
        else "MEDIUM" if confidence_score >= 55
        else "LOW"
    )
    volatility = float(gold.get("volatility", 0) or 0)
    risk = "HIGH" if volatility >= 30 else "MEDIUM" if volatility >= 18 else "LOW"
    regime = "TREND" if abs(float(gold.get("momentum", 0) or 0)) >= 4 else "RANGE"
...
def _gold_source_status(frame):
    if not isinstance(frame, pd.DataFrame) or frame.empty:
        return "UNAVAILABLE"
    return str(frame.attrs.get("status", "LIVE")).upper()


def _gold_navigation():
    items = [
        ("Brief", "gold-brief"),
        ("Health", "gold-health"),
        ("Market", "gold-market"),
    ...
    links += (
        '<a class="pe47-nav-button" '
        'href="?market=oil" target="_self">OIL</a>'
    )
    st.markdown(
        '<div class="pe47-nav-wrap">'
        '<div class="pe47-nav-title">QUICK NAVIGATION</div>'
        '<div class="pe47-nav-grid">' + links + '</div></div>',
        unsafe_allow_html=True
    )


def _gold_explanation(gold, dollar, yield_10y, report):
    reasons = []
    reasons.append(
        "Gold trades above its 20-day moving average."
        if gold.get("trend") == "BULLISH"
        else "Gold trades below its 20-day moving average."
    )
    momentum = float(gold.get("momentum", 0) or 0)
    reasons.append(
        f"10-day momentum is {momentum:+.2f}%."
    )
    dollar_change = float(dollar.get("change", 0) or 0)
    reasons.append(
        f"Dollar Index daily change is {dollar_change:+.2f}% "
        "(normally inverse pressure on gold)."
    )
    yield_change = float(yield_10y.get("change", 0) or 0)
    reasons.append(
        f"US 10-year yield proxy daily change is {yield_change:+.2f}%."
    )
    reasons.append(
        f"Gold-news pressure score is {report.get('news_score', 0):+.1f}."
    )
    return reasons


def run_gold_dashboard():
    gold_df = load_price_series("gold.csv", "GC=F")
    dollar_df = load_price_series("dollar_index.csv", "DX-Y.NYB")
    yield_df = load_price_series("us10y.csv", "^TNX")

    gold = metrics(gold_df)
    dollar = metrics(dollar_df)
    yield_10y = metrics(yield_df)
    news = get_gold_market_news(limit=3)
    report = build_gold_signal(gold, dollar, yield_10y, news)

    pe47_anchor("procureye-top")
    st.markdown(
        '<div class="pe47-executive-header">'
        '<div class="pe47-executive-title">GOLD EXECUTIVE DASHBOARD</div>'
        '<div class="pe47-executive-update">'
        + datetime.now(timezone.utc).strftime("Updated %Y-%m-%d %H:%M UTC")
        + '</div></div>',
        unsafe_allow_html=True
    ...
    st.info(
        "DEV preview · Gold futures in USD (GC=F) are used as the free "
        "global gold-price proxy. No Gold prediction is written to the Oil Gist."
    )

    pe47_anchor("gold-brief")
    section("Gold Daily Market Brief", "One-minute global Gold summary")

    a, b, c, d = st.columns(4)
    with a:
        st.markdown("#### MARKET STRUCTURE")
        st.write(
            f"Gold is **${gold['price']:.2f}** "
            f"({gold['change']:+.2f}%) with a "
            f"**{gold['trend']}** trend."
            if gold.get("price") is not None
            else "Gold price unavailable."
        )
    with b:
        st.markdown("#### MOMENTUM")
        st.write(
            f"Gold 10-day momentum is **{gold.get('momentum', 0):+.2f}%**. "
            f"Annualized 20-day volatility is "
            f"**{gold.get('volatility', 0):.1f}%**."
        )
    with c:
        st.markdown("#### NEWS INTELLIGENCE")
        direction = (
            "BULLISH" if report["news_score"] > 8
            else "BEARISH" if report["news_score"] < -8
            else "NEUTRAL"
        )
        st.write(
            f"Gold-news pressure is **{direction}** with score "
            f"**{report['news_score']:+.1f}**."
        )
        if not news.empty:
            st.caption("Top headline: " + str(news.iloc[0]["Title"]))
    with d:
        st.markdown("#### DECISION")
        st.write(
            f"PROCUREYE indicates **{report['signal']}** with Market Score "
            f"**{report['score']}/100**, confidence **{report['confidence']}**, "
            f"risk **{report['risk']}**, regime **{report['regime']}**."
        )

    s1, s2, s3, s4 = st.columns(4)
    with s1:
        pe47_status_metric("Signal", report["signal"])
    with s2:
        st.metric("Market Score", f"{report['score']}/100")
    with s3:
        st.metric("Signal Confidence", report["confidence"])
    with s4:
        st.metric("Risk", report["risk"])

    _gold_navigation()

    pe47_anchor("gold-health")
    section("Gold System Health", "Free data-source availability")
    h1, h2, h3, h4 = st.columns(4)
    with h1:
        st.metric("Gold Source", _gold_source_status(gold_df))
    with h2:
        st.metric("Dollar Source", _gold_source_status(dollar_df))
    with h3:
        st.metric("US 10Y Source", _gold_source_status(yield_df))
    with h4:
        st.metric("News Items", len(news))
    st.caption(
        "Prices refresh every 5 minutes; Gold news refreshes every 15 minutes. "
        "Fallback data are explicitly identified."
    )

    pe47_anchor("gold-market")
    section("Gold Market Intelligence", "Gold USD proxy interactive 6-month history")
    if gold_df.empty:
        st.warning("Gold market history is temporarily unavailable.")
    else:
        render_professional_chart(gold_df, "Gold Futures in USD", "GOLD")

    pe47_anchor("gold-signal")
    section("Gold Signal Drivers", "Technical, dollar, yield and news contribution")
    driver_table = pd.DataFrame([
        {
            "Driver": "Gold Trend",
            "Value": gold.get("trend", "UNKNOWN"),
            "Direction": "BULLISH" if gold.get("trend") == "BULLISH" else "BEARISH"
        },
        {
            "Driver": "Gold 10D Momentum",
            "Value": f"{gold.get('momentum', 0):+.2f}%",
            "Direction": "BULLISH" if gold.get("momentum", 0) > 0 else "BEARISH"
        },
        {
            "Driver": "Dollar Index",
            "Value": f"{dollar.get('change', 0):+.2f}%",
            "Direction": "BEARISH" if dollar.get("change", 0) > 0 else "BULLISH"
        },
        {
            "Driver": "US 10Y Yield Proxy",
            "Value": f"{yield_10y.get('change', 0):+.2f}%",
            "Direction": "BEARISH" if yield_10y.get("change", 0) > 0 else "BULLISH"
        },
        {
            "Driver": "Gold News",
            "Value": f"{report['news_score']:+.1f}",
            "Direction": (
                "BULLISH" if report["news_score"] > 8
                else "BEARISH" if report["news_score"] < -8
                else "NEUTRAL"
    ...
    st.dataframe(
        pe47_semantic_df(driver_table),
        width="stretch",
        hide_index=True
    )

    pe47_anchor("gold-drivers")
    section("Why This Gold Signal?", "Explainable decision summary")
    for reason in _gold_explanation(gold, dollar, yield_10y, report):
        st.write("• " + reason)

    pe47_anchor("gold-news")
    section("Top Gold-Moving News", "Highest-impact free live Gold headlines")
    if news.empty:
        st.warning("Live Gold news are temporarily unavailable.")
    else:
        for _, item in news.iterrows():
            with st.container(border=True):
                st.markdown("### " + str(item["Title"]))
                st.caption(
                    f"{item['Source']} · {item['Published UTC']} UTC"
                )
                n1, n2 = st.columns(2)
                with n1:
                    pe47_status_metric("Expected Impact", item["Bias"])
                with n2:
                    st.metric("Impact Score", f"{float(item['Impact']):+.1f}")
                if item.get("URL"):
                    st.link_button("Read full article →", str(item["URL"]))

    st.caption(
        "Gold DEV page is decision-support only. It does not execute trades "
        "and does not yet learn from or persist Gold outcomes."
    )


# END PROCUREYE RELEASE 47.6.5.4 GOLD INTELLIGENCE PAGE
...
# ================================================================
# PROCUREYE RELEASE 47.6.5.4 DEV
# Second-page Gold Market Intelligence preview.
# Free sources, explainable signal, no Gold persistence yet.
# Oil production and Oil persistent memory unchanged.
# ================================================================
